...
export const HeroScrollVideo: React.FC<HeroScrollVideoProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (smoothScroll) {
        try {
          // @ts-ignore - lenis is an optional dependency
          const lenisMod = await import(/* webpackIgnore: true */ "lenis").catch(() => null);
          const LenisCtor = lenisMod?.default || (lenisMod as any)?.Lenis;
          if (LenisCtor) {
            lenis = new LenisCtor({
              duration: 0.8,
              smoothWheel: true,
  ...
  const showHeadline = title || subtitle || meta || credits;
  ...
  return (
    <div
      ...
      style={{ ...cssVars, ...style }}
    >
      {/* Headline/hero area - only render if props provided */}
      {showHeadline && (
        <div className="hsv-container" ref={headlineRef}>
          <div className="hsv-headline">
            {title && <h1 className="hsv-title">{title}</h1>}
